# Which JWT signing resources already exist, so migrate mode imports what is
# there and creates what is not. The frozen scripts create none of them, so
# migrating a script-prepared project is the create path. Only read when
...
locals {
  jwt_existing = {
    ...
    signer_sa = try(data.external.jwt_resources[0].result.signer_sa, "false") == "true"
  }
}
