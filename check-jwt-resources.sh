...
# already exist? Each is probed separately because a partly-applied earlier run
# can have some and not others. Absence is distinguished from lookup failure, so
...
SIGNER=$(exists "signer service account" gcloud iam service-accounts describe "${SIGNER_SA}" --project "${PROJECT}" --format="value(email)")

echo "{\"keyring\":\"${KEYRING}\",\"key\":\"${KEY}\",\"role\":\"${ROLE}\",\"signer_sa\":\"${SIGNER}\"}"
